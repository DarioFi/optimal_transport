...
\begin{document}
    ...
    Here we define the theorems that we will use.
        ...
        {\color{red} Put all results in proposition/theorem environments (not subsections) so that you can refer to them later on.}
    ...
    These bounds depend either on the specific instance of the problem $(\mu^{\pm}, N, \alpha)$ or only on the pair $(N, \alpha)$. {\color{red} We care about upper bounds on the number of branches, right?}
    ...
    {\color{red} Define by making the connection with general branched transport in terms of $\mu^-, \mu^+$.}
    ...
    consists of Steiner points {\color{red} why are we calling these Steiner points? Is this terminology used in bracned transport? if yes, why didn't we use this name before?} $s_i \in \R^N$, $E \subseteq S \cup P \times S \cup P$ is the set of edges and
    ...
    \]
        {\color{red} This notation is confusing; are $x,y$ nodes of the graph or the corresponding coordinates of terminals or Steiners? Also, not clear why you are using absolute value since you have not said what a negative flow means in your formulation.}
    \begin{remark}
        The optimal solution of a DBT problem is a union of trees, with a special case where there is only one source and the optimal solution is a tree. {\color{red} why? this follows from Antonio's result, right? if so, cite it.}
    \end{remark}

    \begin{remark}
        The problem is symmetric with respect to changing the sign of the masses, i.e.\ swapping the roles of sources and sinks. {\color{red} Why do we need this remark? Have you used this fact anywhere?}
    \end{remark}
    ...
    We now have a bound on the cosine of the angle between two edges given their flows. In the previous section we have
    taken the supremum over all possible masses to obtain an instance independent bound. {\color{red} Please specify the result you are referring to.} We now focus on an instance
    specific bound, assuming that we already know the set $M$.
        {\color{red} You have a couple of results already depending on $m_1, m_2$. What you do mean we now focus on an instance specific bound? I think you mean you can find better bounds since you made additional assumptions on $\mu^-, \mu^+$, right? (as you describe below). If so, please revise this sentence.}
    ...
    \begin{lemma}
        ...
        Given an instance of a DBT problem with a single source ($p_0$), with masses $M=\{-1, m_1, \dots, m_n\}$ {\color{red} why did you add $-1$ to the set of masses?} such that $\sum_{i =1}^{n} m_i = 1$, terminals $P=\{p_0, p_1, \dots, p_n\}$ and
        a cost parameter $\alpha \in (0,1)$, consider a branching point $s$ with two branches attached to it,
        calling the angle between them $\theta$ and defining $m_{\min} = \min_{i \in P^+} m_i$ we have {\color{red} $P^+$ or $P$?}:
        ...
        \end{equation}
        {\color{red} Recall where $C$ is defined.}
        {\color{red} What does $deg(M, N, \alpha)$ represent? an upper bound on the number of branches? add it.}
    \end{lemma}
    ...
    \begin{corollary}
        ...
        as we know from the conservation of flow that there is at least one angle with both incoming and outgoing flow. {\color{red} I dont understand.}
    \end{corollary}
    \begin{remark}
    ...
    In this section we formulate DBT as an MINLP problem. We first introduce a formulation specific for the case $\alpha=0$
    ...
    \end{align}
    {\color{red} lets always add the constraint that the Steiner points are in the convex hull of terminals.}\\
    Where:
    \begin{itemizeb}
        \leftskip 0.8 cm\labelsep=.3 cm
    ...
    We will prove its correctness by showing that the generalization of this formulation is equivalent to the DBT problem.
        ...
        {\color{red} This is a formulation for DBT, why using the name generalized MMX??}

    \subsection{Generalized MMX}

    We present here a generalization of the MMX formulation that solves a DBT problem with a cost parameter $\alpha$.

    \begin{align}
        \text{(GMMX)}: \; \min &\sum_{[i,j] \in E_1} (f_{ij} + f_{ji})^{\alpha}\|p_i - x^j\| y_{ij} + \sum_{[i,j] \in E_2} (f_{ij} + f_{ji})^{\alpha}\|x^i - x^j\| y_{ij} \\
        & \sum_{j \in S} y_{ij} = 1 \quad \text{for } i \in P, \\
        & \sum_{k < j, k \in S} y_{kj} = 1 \quad \text{for } j \in S - \{p+1\}, \label{gmmx:connectedness} \\
        ...
        & |S| = |P| - 2
    \end{align}
    {\color{red} why $|S| = |P| - 2$ is part of the formulation?}
    {\color{red} Also constrain $x^i$ to be in the convex hull of terminals}
    {\color{red} Get rid of duplicates like $y_{ij}$ and $y_{ji}$.}
    {\color{red} why $f_{ij}$ and $y_{ij}$ are defined over different index sets? The index set over which $f_{ij}$ as defined does not seem correct. It should either be between two Steiners $i,j$ or a Steiner $i$  and a terminal $j$ (similar to what you did in MMX for Steiner).}
    {\color{red} Also please specify under what conditions inequality (6.11) can be replaced by an equality.}
    ...
    \begin{itemizeb}
        ...
        \item [$f_{xy}$] {\color{red} inconsistent notation.} flow on the edge connecting $x$ to $y$
        \item [$y_{xy}$] {\color{red} inconsistent notation.}
        binary variable indicating the presence of an edge between $x$ and $y$
        \item [$deg(\alpha, N, M)$] maximum degree of a vertex in the graph as defined in \ref{lemma:degree_definition}
    \end{itemizeb}
    ...
    \begin{enumerate}
        \item construction of the map that takes feasible solutions of GMMX to graphs that solve DBT
        \item proof that the objective functions are the same
        \item proof of optimality for connected optimal solution where terminals are leaves
        \item proof of optimality for connected optimal solution
        \item proof of optimality for optimal solution
    \end{enumerate}
    ...
    In order to satisfy the connectivity constraint~\ref{gmmx:connectedness} we have to re-order
    ...
    re-order the Steiner points to satisfy the connectivity constraint~\ref{gmmx:connectedness}.
