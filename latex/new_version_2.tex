...
\begin{document}
    ...
    \begin{itemizeb}
        ...
        \item[$|\Omega|$] Lebesgue measure of a set $\Omega$ when it is a subset of $\mathbb{R}$
    ...
    \subsection{ESTP notation} The following notation is specific to the ESTP problem and the MNILP formulations:
    \begin{itemizeb}
        \leftskip 0.8 cm\labelsep=.3 cm
    ...
    Importantly, under certain regularity assumptions, the two energies coincide.
    Todo: more care for the two energy formulations?

    \subsection{Euclidean Steiner Tree problem with masses}

    We can define the ESTP as follows: \\
    ...
    The graph must form a network flow with sources and sinks at the terminal points $P$ (in accordance with their masses),
    such that the following cost function is minimized:
    \[
        \sum_{(x,y) \in E} |f_{xy}|^\alpha \|x - y\|
    \]

    We are particularly interested in the case where there is a single source, so only one terminal has negative mass.
    In this case, the optimal solution is a tree.
    ...
    TODO: I am unsure about the naming of the two problems.
    ...
    Todo: Ask about the general alpha case
    ...
    \begin{lemma}
        ...
        Given an instance of a ESTP problem with a single source ($p_0$), with masses $M=\{-1, m_1, \dots, m_n\}$ such that $\sum_{i =1}^{n} m_i = 1$, terminals $P=\{p_0, p_1, \dots, p_n\}$ and
        ...
        If $\alpha > 1/2$  (assuming we are not optimizing the ratio of the masses, todo: discuss this):
        \begin{equation}
            \cos(\theta) \le \max\left(
            2^{2\alpha - 1} - 1,
            \frac{(1-m_{\min})^{2\alpha} - m_{\min}^{2\alpha} - 1}{2m_{\min}^\alpha}
    ...
    \begin{proof}
        ...
        exist sets $A_1, A_2 \subseteq M$ such that $A_1, A_2 \neq \emptyset$, $m_i = \sum_{k \in A_1} m_k$ and $m_j = \sum_{k \in A_2} m_k$ and $A_1 \cap A_2 = \emptyset$.
    ...
    \begin{itemizeb}
        ...
        \item [$S$] is the set of Steiner $\{s_1, \ldots\}$ points (indexed as $S = \{|P| + 1, ..., 2|P|-2\}$),
    ...
    sink or a source. This comes at a cost, namely some solutions can be unions of disjoint trees and we are imposing
    ...
    Todo: I also wrote the notation for those symbols in the first page, I am not sure on where to put it.

    \subsection{Proof of correctness}
    Todo: understand what I actually want to prove, importantly whether to also consider the case with multiple sources
