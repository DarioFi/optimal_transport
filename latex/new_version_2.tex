...
\begin{document}
    ...
    taken the supremum over all possible masses to obtain an instance independent bound. We now focus on an instance
    ...
    Here we present a formulation for the DBT problem. We first introduce a formulation specific for the case $\alpha=0$
    ...
    \begin{align}
        ...
        + \sum_{i,j \in S} (f_{ij} + f_{ji})^{\alpha}\|x^i - x^j\| y_{ij} \\
        & \sum_{j \in S} y_{ij} = 1 \quad \text{for } i \in P, \\
