...
\begin{document}
    ...
    \begin{align}
        ...
        & c_{ij} \in \R_+^N \quad i \in S, \quad j \in P,
    \end{align}
    {\color{red} lets always add the constraint that the Steiner points are in the convex hull of terminals. DONE}\\
    Where:
    \begin{itemizeb}
        \leftskip 0.8 cm\labelsep=.3 cm
    ...
    We present here a generalization of the MMX formulation that solves a DBT problem with a cost parameter $\alpha$ and a
    single source $p^-=\{p_0\}$.
    ...
    \begin{align}
        ...
        3 \le & \sum_{i \in P} y_{ij} + \sum_{k < j, k \in S} y_{kj} + \sum_{k > j, k \in S} y_{jk} \le deg(\alpha, N, M) \quad \text{for } j \in S \label{dbt_form:degree_bound} \\
        & f_{ij} \le y_{ij} \quad \text{for } [i,j] \in E \\
        & s_i = \sum_{j \in P} c_{ij} p_j \text{    todo: this is an equality for vectors, should I expand it coordinate wise?} \\
    ...
    {\color{red} Also please specify under what conditions inequality \ref{dbt_form:degree_bound} can be replaced by an equality.} \\
    ...
    \begin{itemizeb}
        ...
        \item [$f_{ij}$] flow on the edge connecting $i$ to $j$  {\color{red} inconsistent notation.}
        \item [$y_{ij}$] binary variable indicating the presence of an edge between $i$ and $j$  {\color{red} inconsistent notation.}
        \item [$ deg(\alpha, N, M)$] maximum degree of a vertex in the graph as defined in~\ref{lemma:degree_definition}
    \end{itemizeb}
