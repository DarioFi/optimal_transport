...
# points, pt_c, terminals, edges = point_generator.reduce_to_convex_hull(points, pt_c, terminals, edges)
...
for subset in powerset(points):
    if len(subset) > 1 and len(subset) < len(points):
        ...
        # prob += (pulp.lpSum(x[edge] for edge in edges if all(point in subset for point in edge)) <=
        #          pulp.lpSum(y[point] for point in subset) - 1)

# solve problem and print value
...
#         print(edge)
# for point in points:
#     if pulp.value(y[point]) == 1:
#         print(point)

# print the LP problem
#
# for point in points:
#     print(point, pulp.value(y[point]))
#
# for edge in edges:
#     print(edge, pulp.value(x[edge]))


# plot terminals with red triangles
...
# plot the rest with blue circles
...
# plot selected edges by the LP solution
